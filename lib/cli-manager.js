...
class CliManager {
  ...
  async _waitReady(timeoutMs) {
    ...
    while (Date.now() < deadline) {
      ...
      if (probePort) {
        // CLI variants expose different gateway paths, so probe several:
        //   1. /api/v1/auth/status -> 200  (Windows-bundled CLI gateway)
        //   2. /api/v1/acp          -> 200  (ACP streamable-http, used for login)
        //   3. TCP port reachable         (any HTTP server counts as ready)
        try {
          const { status, body } = await this._httpGet(`http://127.0.0.1:${probePort}/api/v1/auth/status`);
          if (status === 200) {
            try {
              console.log(`[cli] auth/status -> ${JSON.stringify(JSON.parse(body))}`);
            } catch {
              /* ignore */
            }
            return true;
          ...
          /* connection refused -> try next probe */
        }
        try {
          const { status } = await this._httpGet(`http://127.0.0.1:${probePort}/api/v1/acp`);
          if (status === 200) {
            console.log(`[cli] gateway ready via /api/v1/acp (HTTP ${status})`);
            return true;
          }
        } catch {
          /* try TCP */
        }
        if (await tcpProbe(probePort)) {
          console.log(`[cli] gateway ready via TCP port ${probePort}`);
          return true;
        }
      }
...
/** Plain TCP reachability probe (works for any HTTP server). */
