...
function sanitizeSystemPrompt(text) {
  ...
  return String(text)
    ...
    .replace(/Anthropic's official CLI/gi, 'an AI coding assistant')
    .replace(/official CLI for Claude/gi, 'AI coding assistant for the terminal')
    .replace(/You are Claude Code[,.]/gi, 'You are Claude Code')
    // generic "made by Anthropic / Anthropic's product" identity claims
    .replace(/Anthropic (product|assistant|tool|model)/gi, 'an AI $1')
    .replace(/made by Anthropic/gi, 'made by an AI lab');
}
...
function anthropicMessageToOpenAI(m) {
  if (m.role === 'system') {
    // Claude Code inserts in-conversation system messages (system-reminders)
    // into the messages array. Keep them as system role for the OpenAI backend
    // AND sanitize identity-claim phrasings (same as the top-level system).
        ...
        if (Array.isArray(b.content)) {
          content = b.content
            .map((x) => {
              ...
              // Claude Code ToolSearch results come back as tool_reference blocks
              if (x && x.type === 'tool_reference') return `[工具: ${x.tool_name || ''}]`;
              return '';
            })
            .join('');
...
async function handleMessages(_cli, req, res, body, accountOpts) {
  ...
  dumpConverted('messages', messages);
  if (process.env.WORKBUDDY2API_DEBUG !== '0') {
    console.log('[conv] ' + messages
      .map((m) => `${m.role}${m.tool_call_id ? `(id:${m.tool_call_id})` : ''}${m.tool_calls ? `(tcalls:${m.tool_calls.length})` : ''}:${typeof m.content === 'string' ? `str(${m.content.length})` : Array.isArray(m.content) ? 'arr' : typeof m.content}`)
      .join(' | '));
  }
  ...
  try {
    ...
    console.error(`[llm] /v1/messages failed: ${e.message}`);
    sendError(res, e.statusCode || 502, e.code || 'LLM_FAILED', e.message);
  }
}

/** DEBUG=1: dump the converted OpenAI messages to _tools/req_dumps/conv-*.json */
function dumpConverted(tag, messages) {
  if (process.env.WORKBUDDY2API_DEBUG !== '1') return;
  try {
    const fs = require('fs');
    const path = require('path');
    const dir = path.join(__dirname, '..', '_tools', 'req_dumps');
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, `conv-${tag}-${Date.now()}.json`), JSON.stringify(messages, null, 2), 'utf8');
    console.log(`[conv-dump] ${tag} ${messages.length} messages -> _tools/req_dumps/`);
  } catch (e) {
    console.log('[conv-dump] error:', e.message);
  }
}

/** Convert Responses API input[] into OpenAI messages. */
function responsesInputToOpenAI(body) {
  const out = [];
  const instructions = body.instructions ?? body.system;
  if (instructions) {
    const sysText = Array.isArray(instructions)
      ? instructions.map((b) => (typeof b === 'string' ? b : (b && b.text) || '')).join('\n')
      : String(instructions);
    if (sysText) out.push({ role: 'system', content: sanitizeSystemPrompt(sysText) });
  }
  for (const it of body.input || []) {
    ...
    if (it.type === 'function_call') {
      ...
      out.push({ role: 'tool', tool_call_id: it.call_id, content: typeof it.output === 'string' ? it.output : JSON.stringify(it.output) });
    } else if (it.type === 'message' || it.role) {
      const role = it.role === 'assistant' ? 'assistant' : it.role === 'system' ? 'system' : 'user';
      const c = it.content;
      if (typeof c === 'string') {
        out.push({ role, content: role === 'system' ? sanitizeSystemPrompt(c) : c });
      } else if (Array.isArray(c)) {
        const text = c
          .filter((b) => b && typeof b === 'object')
          .map((b) => (b.type === 'text' ? b.text : ''))
          .join('');
        out.push({ role, content: role === 'system' ? sanitizeSystemPrompt(text) : text });
      }
    }
  }
  return out;
...
/** Build a Responses-API output item for accumulated text. */
function respTextItem(id, text) {
  return { type: 'message', id, role: 'assistant', content: text ? [{ type: 'output_text', text, annotations: [] }] : [] };
}

/** Build a Responses-API output item for a tool call. */
function respToolItem(tc) {
  return { type: 'function_call', call_id: tc.id, name: tc.function.name, arguments: tc.function.arguments || '{}' };
}

/**
 * POST /v1/responses — Anthropic Responses API (used by newer Claude Code).
 * @param {object} accountOpts { id, token, userId, endpoint, knownModels }
 */
async function handleResponses(_cli, req, res, body, accountOpts) {
  if (!accountOpts || !accountOpts.token) {
    sendError(res, 401, 'NO_TOKEN', 'account has no valid token');
    return;
  }
  const messages = normalizeDirectMessages(responsesInputToOpenAI(body));
  if (messages.length === 0) {
    sendError(res, 400, 'BAD_REQUEST', 'input must be a non-empty array');
    return;
  }
  dumpConverted('responses', messages);
  const stream = body.stream === true;
  const model = resolveModel(body.model, accountOpts.knownModels);
  const maxTokens = body.max_output_tokens || body.max_tokens || 4096;
  const thinking = body.thinking && body.thinking.type === 'enabled' ? 'high' : body.thinking;
  const tools = anthropicToolsToOpenAI(body.tools);
  const toolChoice = anthropicToolChoiceToOpenAI(body.tool_choice);

  const respId = `resp_${uuid().replace(/-/g, '')}`;
  const modelOut = model || 'default';
  ...
  try {
    if (stream) {
      ...
      const baseResp = { id: respId, object: 'response', created_at: created, status: 'in_progress', model: modelOut, parallel_tool_calls: true };
      res.write(sse('response.created', { type: 'response.created', response: { ...baseResp, output: [] } }));
      res.write(sse('response.in_progress', { type: 'response.in_progress', response: { ...baseResp, output: [] } }));
      ...
      const startTextItem = () => {
        curItemId = `msg_${uuid().replace(/-/g, '')}`;
        curText = '';
        res.write(sse('response.output_item.added', { type: 'response.output_item.added', output_index: outputIndex, item: respTextItem(curItemId, '') }));
        res.write(sse('response.content_block.added', { type: 'response.content_block.added', output_index: outputIndex, content_index: 0, content_block: { type: 'output_text', text: '', citations: [] } }));
      };
      const endTextItem = () => {
        if (!curItemId) return;
        res.write(sse('response.content_block.done', { type: 'response.content_block.done', output_index: outputIndex, content_index: 0, content_block: { type: 'output_text', text: curText, citations: [] } }));
        output.push(respTextItem(curItemId, curText));
        res.write(sse('response.output_item.done', { type: 'response.output_item.done', output_index: outputIndex, item: respTextItem(curItemId, curText) }));
        outputIndex += 1;
        curItemId = null;
      };
      const flushToolItems = () => {
        ...
        for (const idx of [...pendingTools.keys()].sort((a, b) => a - b)) {
          const t = pendingTools.get(idx);
          const item = respToolItem({ id: t.id, function: { name: t.name, arguments: t.args } });
          res.write(sse('response.output_item.added', { type: 'response.output_item.added', output_index: outputIndex, item: { ...item, arguments: '' } }));
          if (t.args) {
            res.write(sse('response.function_call_arguments.delta', { type: 'response.function_call_arguments.delta', output_index: outputIndex, delta: t.args }));
          }
          output.push(item);
      ...
      for await (const d of iter) {
        ...
        if (delta.content) {
          if (!curItemId) startTextItem();
          curText += delta.content;
          res.write(sse('response.output_text.delta', { type: 'response.output_text.delta', output_index: outputIndex, content_index: 0, delta: delta.content }));
        }
        if (choice.finish_reason && !finished) {
          finished = true;
          endTextItem();
      ...
      res.write(sse('response.completed', {
        ...
        response: { id: respId, object: 'response', created_at: created, status: 'completed', model: modelOut, output, parallel_tool_calls: true },
      }));
      res.end();
      return;
    }
    ...
    const msg = (choice && choice.message) || {};
    const output = [];
    if (msg.content) output.push(respTextItem(`msg_${uuid().replace(/-/g, '')}`, msg.content));
    for (const tc of msg.tool_calls || []) output.push(respToolItem(tc));
    sendJson(res, 200, {
      id: respId,
      object: 'response',
      ...
      usage: d.usage || { input_tokens: 0, output_tokens: 0, total_tokens: 0 },
    });
  } catch (e) {
    console.error(`[llm] /v1/responses failed: ${e.message}`);
    sendError(res, e.statusCode || 502, e.code || 'LLM_FAILED', e.message);
  }
}

module.exports = { handleMessages, handleResponses, anthropicMessagesToOpenAI, anthropicToolsToOpenAI, sanitizeSystemPrompt, responsesInputToOpenAI };
