...
function normalizeDirectMessages(messages) {
  ...
  for (const m of messages) {
    ...
    // Keep tool-result messages even with empty content: dropping them would
    // orphan the matching assistant tool_calls and the backend rejects that.
    if (msg.content || msg.tool_calls || msg.role === 'tool') out.push(msg);
  }
  return out;
}
