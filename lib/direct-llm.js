 ...
 *                        thinking?, temperature?, maxTokens?, signal? }
...
function directChat(opts) {
  ...
  // Optional neutral system prompt override (WORKBUDDY2API_SYSTEM_PROMPT):
  // injected only when the client didn't supply its own system message.
  const sysPrompt = process.env.WORKBUDDY2API_SYSTEM_PROMPT;
  if (sysPrompt && !(messages[0] && messages[0].role === 'system')) {
...
async function aggregateStream(iterable) {
  ...
  return { content, reasoning, finishReason, model, usage };
}
