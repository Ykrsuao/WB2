...
function getConfig(runtime) {
  ...
  return {
    ...
    systemPrompt: process.env.WORKBUDDY2API_SYSTEM_PROMPT || env.WORKBUDDY2API_SYSTEM_PROMPT || '',
 ...
 * key (model/effort/systemPrompt/cliArgs) changed.
 */
function applyPatch(patch) {
  const env = readEnv();
  const cliKeys = ['WORKBUDDY2API_MODEL', 'WORKBUDDY2API_EFFORT', 'WORKBUDDY2API_SYSTEM_PROMPT', 'WORKBUDDY2API_CLI_ARGS'];
  const changed = [];
